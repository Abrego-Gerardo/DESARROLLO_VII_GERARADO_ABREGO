...
--Procedimiento para procesar una devolución de producto--
...
BEGIN
    ...
    UPDATE detalles_venta SET cantidad = cantidad - p_cantidad, subtotal = subtotal - v_subtotal
...
--Procedimiento para calcular y aplicar descuentos basados en el historial de compras del cliente--

CREATE PROCEDURE sp_aplicar_descuentos(
    IN p_cliente_id INT,
...
BEGIN
    ...
    -- Aplicar descuento en todas las ventas del cliente
    UPDATE ventas
    SET total = total * (1 - p_porcentaje_descuento / 100)
    WHERE cliente_id = p_cliente_id;
    
    SELECT v_total_compras AS TotalCompras, p_porcentaje_descuento AS PorcentajeDescuento,
    v_total_compras * (1 - p_porcentaje_descuento / 100) AS TotalConDescuento;
END //

--Procedimiento para generar un reporte de productos con bajo stock y sugerir cantidades de reposición--
...
BEGIN
    ...
    read_loop: LOOP
        FETCH cur INTO v_nombre, v_stock;
        IF done THEN
            LEAVE read_loop;
        END IF;
        SET p_reporte = CONCAT(p_reporte, v_nombre, ' tiene bajo stock: ', v_stock, '. Sugerido: ', (10 - v_stock), ' unidades.', '\n');
    END LOOP;
    
    CLOSE cur;
END //

--Procedimiento para calcular comisiones por ventas--
...
BEGIN
    ...
    IF p_criterio = 'monto_total' THEN
        SELECT SUM(total) * 0.05 INTO v_comision FROM ventas WHERE vendedor_id = p_vendedor_id;
    ELSEIF p_criterio = 'cantidad_productos' THEN
        SELECT SUM(dv.cantidad) * 1.00 INTO v_comision 
        FROM ventas v JOIN detalles_venta dv ON v.id = dv.venta_id
        WHERE v.vendedor_id = p_vendedor_id;
    END IF;
    
    SELECT v_comision AS Comision;
END //
